...
# Load grid size
x_max = y_min = None
for _im_id, _x, _y in csv.reader(open(data_directory + 'grid_sizes.csv')):
  if _im_id == IM_ID:
...
train_polygons = None
for _im_id, _poly_type, _poly in csv.reader(open(data_directory + 'train_wkt_v4.csv')):
...
im_rgb = tiff.imread(data_directory + 'three_band/{}.tif'.format(IM_ID)).transpose([1, 2, 0])
